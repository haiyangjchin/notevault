...
用户提问后（回答）：召回（搜索与用户问题相关的片段，计算向量相似度（余弦相似度、欧氏距离、点积），成本低、耗时短、准确率低、适合初步筛选，类似简历筛选）、重排（与召回类似，cross-encoder，成本高、耗时长、准确率高、适合精挑细选，类似面试）、生成（将经过召回和重排后的相关片段与用户问题交给大模型生成最后的答案）

Context-模型输入
Context Window-模型的输入中最多能包含的Token数（1Token≈0.75个单词/1.5汉字）
Context Engineering-